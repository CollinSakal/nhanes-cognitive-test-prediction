...
# Data
df_final = pl.read_csv('data/nhanes/derived/df-final-nhanes.csv', null_values=['NA'], infer_schema_length=1500)
...
for i in range(nrepeats):
    ...
    for k in range(nfolds):
        ...
        x_train = df.filter(pl.col('fold') != k).select(X_cols).to_numpy()
        x_valid = df.filter(pl.col('fold') == k).select(X_cols).to_numpy()

        x_train = imputer.fit_transform(x_train)
        x_valid = imputer.transform(x_valid)

        x_train = pl.from_numpy(x_train, schema=X_cols)
        x_valid = pl.from_numpy(x_valid, schema=X_cols)

        # Save
        x_train.write_csv(f'data/nhanes/derived/cv-folds/x-train-{niter}.csv')
        x_valid.write_csv(f'data/nhanes/derived/cv-folds/x-valid-{niter}.csv')

        y_train.write_csv(f'data/nhanes/derived/cv-folds/y-train-{niter}.csv')
        y_valid.write_csv(f'data/nhanes/derived/cv-folds/y-valid-{niter}.csv')
