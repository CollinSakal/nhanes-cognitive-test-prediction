...
for i in range(1,nfolds*nrepeats+1):
    ...
    #cat_preds_cerad_train = model_cerad.predict_proba(pool_train_cerad)
    #cat_preds_af_train = model_af.predict_proba(pool_train_af)
    #cat_preds_dsst_train = model_dsst.predict_proba(pool_train_dsst)

    # Initialize and fit calibrators
    #calibrator_cerad = IsotonicRegression(out_of_bounds='clip').fit(cat_preds_cerad_train[:, 1], y_train_cerad)
    #calibrator_af = IsotonicRegression(out_of_bounds='clip').fit(cat_preds_af_train[:, 1], y_train_af)
    #calibrator_dsst = IsotonicRegression(out_of_bounds='clip').fit(cat_preds_dsst_train[:, 1], y_train_dsst)

    # Make validation set predictions with CatBoost
    #cat_preds_cerad_valid = model_cerad.predict_proba(pool_valid_cerad)
    #cat_preds_af_valid = model_af.predict_proba(pool_valid_af)
    #cat_preds_dsst_valid = model_dsst.predict_proba(pool_valid_dsst)

    # Get final predictions from the calibrators
    #preds_cerad = calibrator_cerad.predict(cat_preds_cerad_valid[:, 1])
    #preds_af = calibrator_af.predict(cat_preds_cerad_valid[:, 1])
    #preds_dsst = calibrator_dsst.predict(cat_preds_cerad_valid[:, 1])
...
df_output.write_csv(f'results/performance-metrics/internal-validation-wearable-based-cat.csv')
